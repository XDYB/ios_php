...
var in_id = 0;
var in_size = '<?php echo '3072';//intval(ini_get('upload_max_filesize'));?>';
				...
				<div class="surplus"> 
				    <div class="apps-app-security">
                        <div class="btn-invite-member" style="margin-left:5px;border:1px solid red;color:#000fff">
                          <a href="http://wpa.qq.com/msgrd?v=3&uin=36660365&site=qq&menu=yes" target="view_window"><font color="red">有任何问题点此联系站长</font></a>
                        </div>
                    </div>
...
while ($row = $GLOBALS['db']->fetch_array($query)) {
   ...
   // }
    ...
    echo '</tbody></table>';
    echo '<div class="action"><a class="ng-binding" href="' . IN_PATH . 'index.php/profile_app/' . $row['in_id'] . '"><i class="icon-pen"></i> 管理</a><a href="' . $GLOBALS['shorturl']->GetShortUrl(getlink($row['in_id'])) . '" target="_blank" class="ng-binding"><i class="icon-eye"></i> 预览</a><button class="btn btn-remove ng-scope" onclick="del_app(' . $row['in_id'] . ', 1)"><i class="icon icon-trash"></i></button></div>';
    echo '</div></div>';
}?>
</div>
</div>
